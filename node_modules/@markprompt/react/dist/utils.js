...
export function markdownToString(markdown = '', maxLength = 200) {
 ...
 * why is that useful.
 */
export function hasPresentKey(k) {
    return function (a) {
 ...
 * why is that useful.
 */
export function hasValueAtKey(k, v) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return function (a) {
        return a[k] === v;
    };
}
export const isStoredError = (value) => {
 ...
 */
export function getDefaultView(userProvidedDefaultView, options) {
    const isSearchEnabled = options?.search?.enabled ?? DEFAULT_MARKPROMPT_OPTIONS.search.enabled;
    const isChatEnabled = options?.chat?.enabled ?? DEFAULT_MARKPROMPT_OPTIONS.chat.enabled;
